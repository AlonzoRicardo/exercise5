...
const handleRollBackToBases = require("../handlers/handleRollBackToBases");
//validators
...
const { hierarchy } = require("../db/dbListeners");
...
class ServiceController {
  ...
  sendMessage(destination, body, uuid, res) {
    this.service
      .post("/message", { destination, body, uuid })
      .then(() => {
        this.updateMessageStatus(uuid, true, true).then(resp => {
          res.status(200).send(resp);
        });
      })
      .catch(err => {
        err.message == `timeout of ${delay}ms exceeded`
          ? this.updateMessageStatus(uuid, true, false).then(() => {
  ...
  rollBackCredits() {
    handleRollBackToBases();
    if (hierarchy.length > 1) {
      handleRollBackToBases(slaveDB(hierarchy), "db2");
      handleRollBackToBases(leaderDB(hierarchy), "main db");
    } else {
      handleRollBackToBases(leaderDB(hierarchy), "main db");
    }
  }
